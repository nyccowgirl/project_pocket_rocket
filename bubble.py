...
def bubble_data():
    """Creates and jsonify datasets for bubble chart. Writes data to file and returns
    nothing."""
    ...
    with open('static/js/bubble_data.js', 'w') as outfile:
        json.dump(datasource, outfile, indent=2)

    with open('static/js/bubble_data.js', 'r+') as f:
        content = f.read()
        f.seek(0, 0)
        f.write('let dataSource = ' + content)

    with open('static/js/bubble_data.js', 'a') as f:
        f.write(';')
